...
void parse_command(char* cmd, char* c) {
    ...
    if (strcmp(cmd, "product_uuid")==0) {
        ...
        if ((vinf->serial_4==0) && (vinf->serial_3==0) && (vinf->serial_2==0) && (vinf->serial_1==0)) {
            ...
            QByteArray tempUUID = uuidGen.createUuid().toByteArray();

            memcpy(&vinf->serial_1, tempUUID.constData(), 16);
...
int main(int argc, char *argv[]) {
    ...
    for (j = 0; j < 1000; j++)
    {
        ...
        QString number = QString("%1").arg(fileCounter, 6, 10, QChar('0'));

        fileName.append(number);
        fileName.append(".hex");
        QByteArray ba = fileName.toLocal8Bit();
        ret = write_binary(ba.data() );
        if (ret) {
